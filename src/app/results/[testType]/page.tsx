...
- Total Points: ${decisionMakingResults.totalPoints}/${decisionMakingResults.maxPossiblePoints}
- Effectiveness Score: ${decisionMakingResults.accuracy}%
...
${Object.entries(decisionMakingResults.categoryScores).map(([category, score]) => {
  ...
  const totalCount = categoryQuestions.length;
  const maxPossiblePoints = totalCount * 100;
  const earnedPoints = score as number;
  const percentage = maxPossiblePoints > 0 ? Math.round((earnedPoints / maxPossiblePoints) * 100) : 0;
  return `- ${category}: ${percentage}% (${earnedPoints}/${maxPossiblePoints} pts)`;
}).join('\n')}
...
${(() => {
  ...
  decisionMakingQuestions.forEach((question: any) => {
    const userAnswer = decisionMakingAnswers[question.id];
    if (userAnswer !== undefined && question.options && question.options[userAnswer]) {
      const points = question.options[userAnswer].weight;
      if (points >= 90) excellent++;
      else if (points >= 70) good++;
      else if (points >= 50) fair++;
      else poor++;
    } else {
      poor++;
    }
  ...
  const total = decisionMakingQuestions.length;
  return `- Excellent (90-100 pts): ${Math.round((excellent / total) * 100)}% (${excellent}/${total})
- Good (70-89 pts): ${Math.round((good / total) * 100)}% (${good}/${total})
- Fair (50-69 pts): ${Math.round((fair / total) * 100)}% (${fair}/${total})
- Poor (0-49 pts): ${Math.round((poor / total) * 100)}% (${poor}/${total})`;
})()}

Speed Analysis:
${(() => {
  const answerTimes = JSON.parse(searchParams.get('answerTimes') || '{}');
  let lightningFast = 0, quick = 0, slow = 0;
  
  decisionMakingQuestions.forEach((question: any) => {
    const responseTime = answerTimes[question.id] || 0;
    if (responseTime < 10) lightningFast++;
    else if (responseTime < 20) quick++;
    else slow++;
  });
  
  const total = decisionMakingQuestions.length;
  return `- Lightning Fast (<10s): ${Math.round((lightningFast / total) * 100)}% (${lightningFast}/${total}) - No penalty
- Quick (10-20s): ${Math.round((quick / total) * 100)}% (${quick}/${total}) - 10% penalty
- Slow (20+s): ${Math.round((slow / total) * 100)}% (${slow}/${total}) - 20% penalty`;
})()}
...
${decisionMakingQuestions.map((q: any, index: number) => {
  ...
  const userAnswerText = userAnswer !== undefined && q.options[userAnswer] ? q.options[userAnswer].text : 'Not answered'
  const userAnswerWeight = userAnswer !== undefined && q.options[userAnswer] ? q.options[userAnswer].weight : 0
  const bestAnswer = q.options.find((opt: any) => opt.weight === Math.max(...q.options.map((o: any) => o.weight)))
  ...
  const penalty = speedMultiplier === 1.0 ? 'No penalty' : speedMultiplier === 0.9 ? '10% penalty' : '20% penalty';
  
  return `Question ${index + 1}: ${q.question}
Category: ${q.category}
Your Answer: ${userAnswerText} (${userAnswerWeight} points)
Best Answer: ${bestAnswer ? bestAnswer.text : 'Unknown'}
Effectiveness: ${userAnswerWeight >= 90 ? 'Excellent' : userAnswerWeight >= 70 ? 'Good' : userAnswerWeight >= 50 ? 'Fair' : 'Poor'}
Response Time: ${responseTime}s (${speedLevel}) - ${penalty}
Speed Score: ${Math.round(speedPoints)} points
...
Based on: ${creativityResults.totalQuestions} creative scenarios

What This Means:
${creativityResults.description}

Your Creative Preferences:
🎨 Innovation: ${creativityResults.innovationPreference}%
🔬 Research: ${creativityResults.researchPreference}%
⚙️ Practical: ${creativityResults.practicalPreference}%
...
${creativityQuestions.map((q: any, index: number) => {
  const userRanking = creativityAnswers[q.id] || []
  const rankedOptions = userRanking.map((rankIndex: number, rank: number) => {
    const option = q.options[rankIndex]
    const optionText = typeof option === 'string' ? option : option?.text || 'Unknown option'
    const optionType = typeof option === 'string' ? 'Unknown' : option?.type || 'Unknown'
    return `   ${rank + 1}. [Option ${rankIndex + 1}] ${optionText} (${optionType})`
  }).join('\n')
...
Category: ${q.options[0]?.type ? q.options[0].type.charAt(0).toUpperCase() + q.options[0].type.slice(1) : 'Unknown'}
`
}).join('')}
...
Visit https://testyourself.com for more tests!`
    ...
    return (
      <div className="space-y-2 mb-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-2">
          {/* Your Results - 40% width */}
          <div className="lg:col-span-2">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              <div className="text-center">
                <h2 className="text-xl text-blue-800 mb-1">
                  <span className="font-bold">Your Results</span>
                </h2>
          ...
          {/* Key Insights - 60% width */}
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Key Insights</h3>
              <div className="space-y-3">
                <p className="text-gray-700">{results.description}</p>
    ...
    return (
      <div className="space-y-2 mb-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-2">
          {/* Your Results - 40% width */}
          <div className="lg:col-span-2">
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              <div className="text-center">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(results.categoryScores).map(([category, percentage]) => {
              // For World Geography, categoryScores already contains percentages
              ...
              return (
                <div key={category} className="bg-gray-100 rounded-lg p-2">
  ...
  const renderDecisionMakingResults = () => {
    const results = JSON.parse(searchParams.get('results') || '{}')
    const answers = JSON.parse(searchParams.get('answers') || '{}')
    const questions = JSON.parse(searchParams.get('questions') || '[]')
    ...
    return (
      <div className="space-y-2 mb-2">
          ...
          <div className="lg:col-span-2">
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              <div className="text-center">
                ...
                <p className="text-lg text-green-700 mb-3">Here's what we discovered about your<br /><span className="font-bold">Decision Making Skills</span></p>
                <div className="text-5xl font-bold text-green-600 mb-2">{results.accuracy}%</div>
                <div className="text-2xl font-semibold text-gray-800 mb-2">{results.level}</div>
                <div className="text-xl font-semibold text-green-700">{results.totalPoints}/{results.maxPossiblePoints} points</div>
                <div className="text-lg font-semibold text-blue-600 mb-1">
                  Speed Score: {results.speedScore}%
                </div>
                <div className="text-sm text-green-600">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              ...
              <div className="space-y-3">
                ...
                <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                  <div className="text-sm font-medium text-green-700 mb-1">🧠 What This Means for You:</div>
                  <div className="text-green-800 text-sm">
                    Your decision-making skills show {results.accuracy >= 80 ? 'strong' : results.accuracy >= 60 ? 'solid' : 'developing'} judgment and reasoning abilities. 
                    {results.accuracy >= 80 ? ' You demonstrate excellent decision-making across various scenarios and situations.' : 
                     results.accuracy >= 60 ? ' You have strong foundational decision-making skills with room for refinement.' : 
                     ' Consider practicing decision-making exercises and studying decision-making frameworks.'}
                  </div>
                </div>
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                  <div className="text-sm font-medium text-blue-700 mb-1">🎯 Practical Applications:</div>
                  <div className="text-blue-800 text-sm">
                    {results.accuracy >= 80 ? 'Your strong decision-making skills can help with leadership roles, complex problem-solving, and strategic planning in both personal and professional contexts.' :
                     results.accuracy >= 60 ? 'Your decision-making skills provide a good foundation for handling everyday decisions and can be developed further for more complex situations.' :
                     'Building decision-making skills can enhance your ability to handle challenges, make better choices, and improve your judgment in various situations.'}
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(results.categoryScores).map(([category, score]) => {
              // Calculate points earned vs max possible for this category
              const categoryQuestions = questions.filter(q => q.category === category);
              const totalCount = categoryQuestions.length;
              const maxPossiblePoints = totalCount * 100; // 100 points per question
              const earnedPoints = score as number;
              const percentage = maxPossiblePoints > 0 ? Math.round((earnedPoints / maxPossiblePoints) * 100) : 0;
              ...
              return (
                <div key={category} className="bg-gray-100 rounded-lg p-2">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-medium text-gray-700">{category}</span>
                    <span className="text-sm font-bold text-green-600">{percentage}% ({earnedPoints}/{maxPossiblePoints} pts)</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5">
                    <div 
                      className="bg-green-500 h-1.5 rounded-full transition-all duration-300"
        ...
        {/* Point Range Distribution */}
        <div className="bg-white rounded-2xl shadow-lg p-3">
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Answer Quality Distribution</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {(() => {
              // Calculate distribution of answers by point ranges
              let excellent = 0, good = 0, fair = 0, poor = 0;
              ...
              questions.forEach((question: any) => {
                ...
                if (userAnswer !== undefined && question.options && question.options[userAnswer]) {
                  ...
                  poor++; // Unanswered questions count as poor
                }
              });
              ...
              return [
                { label: 'Excellent', count: excellent, percentage: Math.round((excellent / total) * 100), color: 'bg-green-500', textColor: 'text-green-800', range: '90-100 pts' },
                { label: 'Good', count: good, percentage: Math.round((good / total) * 100), color: 'bg-yellow-500', textColor: 'text-yellow-800', range: '70-89 pts' },
                { label: 'Fair', count: fair, percentage: Math.round((fair / total) * 100), color: 'bg-orange-500', textColor: 'text-orange-800', range: '50-69 pts' },
                { label: 'Poor', count: poor, percentage: Math.round((poor / total) * 100), color: 'bg-red-500', textColor: 'text-red-800', range: '0-49 pts' }
              ].map((range) => (
                <div key={range.label} className="bg-gray-100 rounded-lg p-2">
                  <div className="flex justify-between items-center mb-1">
                    <span className={`text-sm font-medium ${range.textColor}`}>{range.label} ({range.range})</span>
                    <span className={`text-sm font-bold ${range.textColor}`}>{range.percentage}% ({range.count}/{total})</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5">
                    <div 
                      className={`${range.color} h-1.5 rounded-full transition-all duration-300`}
  ...
  const renderTimeManagementResults = () => {
    ...
    return (
      <div className="space-y-2 mb-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-2">
          {/* Your Results - 40% width */}
          <div className="lg:col-span-2">
            <div className="bg-gradient-to-r from-green-50 to-purple-50 rounded-2xl shadow-lg p-3 h-full flex flex-col justify-center">
              <div className="text-center">
                <h2 className="text-xl text-green-800 mb-1">
                  <span className="font-bold">Your Results</span>
                </h2>
    ...
    const renderCreativityTestResults = () => {
      const results = JSON.parse(searchParams.get('results') || '{}')
      const answers = JSON.parse(searchParams.get('answers') || '{}')
      const questions = JSON.parse(searchParams.get('questions') || '[]')
      ...
      return (
        <div className="space-y-2 mb-2">
          {/* Your Creative Type and What This Means - Side by Side */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-2">
            {/* Your Creative Type */}
            <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-2xl shadow-lg p-2">
              <div className="text-center">
                <h2 className="text-2xl text-pink-800 mb-1">
                  <span className="font-bold">Your Creative Type</span>
                </h2>
                <div className="text-4xl font-bold text-pink-600 mb-2">{results.creativityType}</div>
                <p className="text-lg text-pink-700 mb-2">Based on your responses to {results.totalQuestions} creative scenarios</p>
              </div>
            </div>

            {/* What This Means */}
            <div className="bg-white rounded-2xl shadow-lg p-4 flex flex-col justify-center">
              <h3 className="text-lg font-semibold text-gray-800 mb-1">What This Means</h3>
              <p className="text-gray-700 leading-relaxed">{results.description}</p>
            </div>
          </div>

          {/* Creative Preferences Breakdown */}
          <div className="bg-white rounded-2xl shadow-lg p-4 mb-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Your Creative Preferences</h3>
            <p className="text-sm text-gray-600 mb-4">Based on which options you ranked highest across all questions</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-gray-700">🎨 Innovation</span>
                  <span className="text-sm font-bold text-pink-600">{results.innovationPreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-pink-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.innovationPreference}%` }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-purple-600">{results.researchPreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-purple-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.researchPreference}%` }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-gray-700">⚙️ Practical</span>
                  <span className="text-sm font-bold text-blue-600">{results.practicalPreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.practicalPreference}%` }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-green-600">{results.boldPreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-green-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.boldPreference}%` }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-orange-600">{results.collaborativePreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-orange-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.collaborativePreference}%` }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-indigo-600">{results.intuitivePreference}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-indigo-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${results.intuitivePreference}%` }}
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-4">
            {/* Strengths */}
            <div className="bg-white rounded-2xl shadow-lg p-4">
              <h3 className="text-lg font-semibold text-gray-800 mb-3">Your Creative Strengths</h3>
              <div className="space-y-3">
                {results.strengths.map((strength: string, index: number) => (
                  <div key={index} className="flex items-start">
                    <span className="text-pink-500 mr-2 text-sm">•</span>
            ...
            {/* Areas for Growth */}
            <div className="bg-white rounded-2xl shadow-lg p-4">
              <h3 className="text-lg font-semibold text-gray-800 mb-3">Areas for Development</h3>
              <div className="space-y-3">
                {results.areasForGrowth.map((area: string, index: number) => (
                  <div key={index} className="flex items-start">
  ...
  const getSEOContent = () => {
    ...
    switch (testType) {
      ...
      case 'decision-making-test':
        return {
          title: 'Decision Making Test Results - Advanced Weighted Assessment | TestYourself',
          description: 'View your decision making test results with advanced weighted scoring! Discover your judgment effectiveness, speed penalty analysis, and performance across workplace leadership, personal decisions, ethical dilemmas, crisis management, and strategic thinking.',
          keywords: 'decision making test results, weighted scoring results, effectiveness points, speed penalties, judgment assessment results, leadership decisions results, ethical dilemmas results, crisis management results, strategic thinking results',
          ogTitle: 'Decision Making Test Results - Advanced Weighted Assessment',
          ogDescription: 'View your decision making test results with advanced weighted scoring! Discover your judgment effectiveness, speed penalty analysis, and performance across different decision-making scenarios.',
          ogImage: `${baseUrl}/decision-making-test-results-og-image.jpg`,
          canonical: `${baseUrl}/results/decision-making-test`
        }
      case 'creativity-test':
        return {
          title: 'Creativity Explorer Results - Creative Thinking Assessment | TestYourself',
          description: 'View your creativity test results! Discover your creative type (Specialist, Combination, or Balanced), detailed preference percentages for Innovation, Research, Practical, Bold, Collaborative, and Intuitive thinking styles with comprehensive analysis.',
          keywords: 'creativity test results, creative thinking results, innovation assessment results, artistic expression results, creative type results, problem-solving creativity results, innovation preference, research preference, practical creativity, bold creativity, collaborative creativity, intuitive creativity, creative preferences, specialist creative, combination creative, balanced creative, ranking-based creativity assessment',
          ogTitle: 'Creativity Explorer Results - Creative Thinking Assessment',
          ogDescription: 'View your creativity test results! Discover your creative type and detailed preference analysis across Innovation, Research, Practical, Bold, Collaborative, and Intuitive thinking styles.',
          ogImage: `${baseUrl}/creativity-test-results-og-image.jpg`,
          canonical: `${baseUrl}/results/creativity-test`
        }
      default:
        return {
    ...
    <>
      ...
      <div className="pt-2 px-4 sm:px-6 lg:px-8 flex-grow">
        ...
        <Header onLogoClick={undefined} />
            ...
            <div className="bg-white rounded-2xl shadow-lg px-2 py-0.5">
                <h1 className="text-lg font-bold text-gray-800">
                  {testType === 'personality' ? 'Character Assessment Results' :
                   ...
                   testType === 'decision-making-test' ? 'Decision Making Test Results' :
                   testType === 'creativity-test' ? 'Creativity Explorer Results' :
                   'Test Results'}
                </h1>
            </div>
          </div>
            ...
            {/* Results Content */}
            <div className={`mt-2 ${(testType === 'emotional-intelligence' || testType === 'personality' || testType === 'optical-illusion' || testType === 'trivia' || testType === 'memory' || testType === 'math' || testType === 'color-blindness' || testType === 'reaction-time' || testType === 'stress-test' || testType === 'iq-test' || testType === 'time-management-test' || testType === 'spatial-reasoning-test' || testType === 'world-geography-test' || testType === 'decision-making-test') ? 'mb-2' : ''}`}>
            {testType === 'personality' ? renderPersonalityResults() : 
             ...
             testType === 'decision-making-test' ? renderDecisionMakingResults() :
             testType === 'creativity-test' ? renderCreativityTestResults() :
             renderOtherResults()}
          </div>
